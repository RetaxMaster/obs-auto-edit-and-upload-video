...
static void test_three_inputs_nvenc_h264()
{
	...
	for (const auto &a : args)
		if (a.find("concat=n=3") != std::string::npos) {
			found_concat = true;
			break;
		}
	assert(found_concat);
...
static void test_input_only_software_hevc()
{
	...
	bool found_concat = false;
	for (const auto &a : args)
		if (a.find("concat") != std::string::npos) {
			found_concat = true;
			break;
		}
	assert(!found_concat);
...
static void test_intro_only()
{
	...
	bool found_concat = false;
	for (const auto &a : args)
		if (a.find("concat=n=2") != std::string::npos) {
			found_concat = true;
			break;
		}
	assert(found_concat);
...
static void test_resolution_scaling_applied()
{
	...
	for (const auto &a : args)
		if (a.find("scale=1280:720") != std::string::npos) {
			found_scale = true;
			break;
		}
	assert(found_scale);
