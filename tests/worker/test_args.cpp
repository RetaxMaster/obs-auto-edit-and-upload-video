...
{
	const char *argv[] = {
		"worker",     "--input", "/tmp/rec.mkv", "--output", "/tmp/out.mp4", "--ffmpeg", "/usr/bin/ffmpeg",
		"--vcodec",   "h264",    "--encoder",    "nvenc",    "--bitrate",    "6000",     "--progress",
		"/tmp/p.txt", "--width", "1280",         "--height", "720",          "--format", "mkv"};
	int argc = 21;
	WorkerArgs args = parse_args(argc, const_cast<char **>(argv));
...
static void test_resolution_and_format_required()
{
	...
	const char *argv[] = {
		"worker",     "--input",  "/tmp/rec.mkv", "--output", "/tmp/out.mp4", "--ffmpeg", "/usr/bin/ffmpeg",
		"--vcodec",   "h264",     "--encoder",    "software", "--bitrate",    "8000",     "--progress",
		"/tmp/p.txt", "--height", "1080",         "--format", "mp4"};
	int argc = 19;
	bool threw = false;
	try {
		parse_args(argc, const_cast<char **>(argv));
